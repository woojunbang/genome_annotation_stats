...
def parse_annotation_file(annotation_file, file_format):
    gene_count = 0
    gene_lengths = []
    exon_count = 0
    gene_types = {}
                ...
                if len(fields) == 9:
                    ...
                    if feature_type == 'gene':
                        ...
                        if file_format == 'gff3':
                            ...
                            gene_id = [x.split(' ')[1].replace('"', '') for x in fields[8].split(';') if 'gene_id' in x][0]
                        
                        gene_types[gene_id] = gene_types.get(gene_id, []) + [cds_length]
                        cds_per_gene.append(gene_id)

    if gene_count == 0:
        raise ValueError("No gene features found in the annotation file.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--annotation", required=True, help="Path to the annotation file (GFF3 or GTF).")
    ...
    parser.add_argument("--format", required=True, choices=['gff3', 'gtf'], help="Format of the annotation file (gff3 or gtf).")
    args = parser.parse_args()

    genome_size, gc_content = calculate_genome_statistics(args.genome)
    gene_count, average_gene_length, exon_count, average_exons_per_gene, average_cds_length, cds_per_gene_count = parse_annotation_file(args.annotation, args.format)
